...
import { useAppAgent } from "@/app/assistant";

// Agent selection context (use app-level context)
...
const AgentSelector: FC = () => {
  ...
  return (
    <div className="w-full max-w-[var(--thread-max-width)] mb-4">
...
const ThreadWelcomeSuggestions: FC = () => {
  const { selectedAgent } = useAppAgent();

  const suggestions = {
    researchAgent: [
      {
        prompt: "What problems does sequence modeling face with neural networks?",
        text: "Sequence modeling challenges"
      },
      {
        prompt: "Explain the transformer architecture and its key innovations",
    ...
    weatherAgent: [
      ...
      {
        prompt: "Plan outdoor activities for San Francisco this weekend",
        text: "Weekend activity planning"
      }
    ]
  };

  const currentSuggestions = suggestions[selectedAgent];
  ...
  return (
    ...
    >
      <ComposerPrimitive.Input
        rows={1}
        autoFocus
        placeholder={`Write a message to ${selectedAgent === 'researchAgent' ? 'Research Assistant' : 'Weather Agent'}...`}
